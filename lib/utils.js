...
function settings (options) {
  ...
  returns.test = is(options.test, 'RegExp') ?
    options.test :
    RegExp('^' + escape(returns.fence[0]) + '(.*?)' + escape(returns.fence[1]))

  return returns
}

// Escape chars in regex string explained here https://tinyurl.com/ydagqbrc
function escape (s) {
  return s.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')
}
